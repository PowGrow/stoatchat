...
//!
//! Fork of rocket_authifier::routes::account::create_account which rejects
//! invites that have already been claimed.
...
pub async fn create_account(
    ...
    let invite = if authifier.config.invite_only {
        if let Some(invite) = data.invite {
            let invite = authifier.database.find_invite(&invite).await?;

            // Upstream only looks the invite up, so a single code could be
            // claimed any number of times; refuse the ones already spent.
            if invite.used {
